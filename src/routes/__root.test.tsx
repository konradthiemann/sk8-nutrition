...
describe("app shell", () => {
  ...
  it("shows the app name and the contest countdown in the header", async () => {
    ...
    expect(within(header).getByRole("link", { name: "SK8 Ernährung" })).toHaveAttribute(
      "href",
      "/",
    );
  ...
  it("navigates via the bottom navigation and records the screen change", async () => {
    ...
    await user.click(within(nav).getByRole("link", { name: "Hydration" }));
    expect(await screen.findByRole("heading", { level: 1, name: "Hydration" })).toBeInTheDocument();
    expect(telemetry.currentScreen).toBe("/hydration");
    ...
    expect(events[1]).toMatchObject({
      screen: "/",
      target: "/hydration",
      meta: { from: "/", to: "/hydration", via: "bottom-nav" },
    });
    expect(events[2]).toMatchObject({ screen: "/", meta: { ms: expect.any(Number) } });
    expect(events[3]).toMatchObject({ screen: "/hydration", meta: { from: "/" } });
  });
